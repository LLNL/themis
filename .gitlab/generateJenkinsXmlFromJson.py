...
#    print line
  ...
  for testresult in tests['PASSED']:
    fil.write('    <testcase status="run" time="%.3f" classname="%s" name="%s"/>\n' % ( testresult.getElapsedTime(), testresult.getClassname(), testresult.getName()) )
  for testresult in tests['NOT_PASSED']:
    fil.write('    <testcase time="%.3f" classname="%s" name="%s">\n' % (testresult.getElapsedTime(), testresult.getClassname(), testresult.getName()) )
    fil.write('      <failure type="%s"> "%s" </failure>\n' % (testresult.getStatus(),testresult.getResultDesc()) )
    fil.write('    </testcase>\n')
  fil.write('  </testsuite>\n')
  fil.write('</testsuites>\n')

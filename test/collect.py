...
def setup_parser():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Combine test suite results, " "then print them and write them to a file."
        ),
    )
    parser.add_argument(
        "suites", help="the test suites to collect from", type=str, nargs="*"
    )
    parser.add_argument(
        "-d",
        "--directories",
...
def main():
    ...
    for suite in args.suites:
        for directory in args.directories:
            test_files.extend(
                glob.glob(os.path.join(directory, utils.RESULT_FILE.format(suite, "*")))
            )
    results = []
    ...
    print()
    print(
        "Tests run: {}, skips: {}, expected failures: {}".format(
    ...
    print(
        "Total failures: {}, errors: {}, unexpected successes: {}".format(

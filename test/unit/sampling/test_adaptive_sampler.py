...
import uqp.sampling.adaptive_sampler
...
class TestAdaptiveSamplers(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.np_train_input = uqp.sampling.sampler.LatinHyperCubeSampler.sample_points(num_points=200,
                                                                                   box=ls_test_box,
                                                                                   seed=2018)
        self.np_train_output = helper_function(self.np_train_input)
        self.surrogate_model = uqp.surrogate_model.surrogate_model.SurrogateModel('gpr', ['x1', 'x2', 'x3'], ['Y'],
                                                                                  ls_test_box).fit(self.np_train_input,
                                                                                                   self.np_train_output)
        self.np_candidate_points = uqp.sampling.sampler.LatinHyperCubeSampler.sample_points(num_points=200,
                                                                                        box=ls_test_box,
                                                                                        seed=2019)

    def test_ActiveLearningSampler_valid(self):
        np_actual_values = uqp.sampling.adaptive_sampler.\
    ...
    def test_ActiveLearningSampler_invalid(self):
        ...
        self.assertRaises(TypeError, uqp.sampling.adaptive_sampler.ActiveLearningSampler.sample_points,
                          cand_points=self.np_candidate_points,
                          model=self.surrogate_model)
    ...
    def test_DeltaSampler_invalid(self):
        # num_points not given
        self.assertRaises(TypeError, uqp.sampling.adaptive_sampler.DeltaSampler.sample_points,
                          cand_points=self.np_candidate_points,
                          model=self.surrogate_model,
                          X=self.np_train_input,
                          Y=self.np_train_output)
    ...
    def test_ExpectedImprovementSampler_invalid(self):
        # num_points not given
        self.assertRaises(TypeError, uqp.sampling.adaptive_sampler.ExpectedImprovementSampler.sample_points,
                          cand_points=self.np_candidate_points,
                          model=self.surrogate_model,
                          X=self.np_train_input,
                          Y=self.np_train_output)
    ...
    def test_LearningExpectedImprovementSampler_invalid(self):
        # num_points not given
        self.assertRaises(TypeError, uqp.sampling.adaptive_sampler.LearningExpectedImprovementSampler.sample_points,
                          cand_points=self.np_candidate_points,
                          model=self.surrogate_model,
                          X=self.np_train_input,
                          Y=self.np_train_output)

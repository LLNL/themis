...
import themis
from uqp.sampling import composite_samples, sampler
from uqp.surrogate_model import surrogate_model
from uqp.uq_component import uqp_mcmc, plots
...
for i, (X_ob, Y_ob) in enumerate(zip(X_obs, Y_obs)):
    ...
    model = surrogate_model.SurrogateModel(MODEL_TYPE, ['a', 'b', 'c'], ['Y'],
                                           Samples.get_ranges(['a', 'b', 'c'], scaled=False))
    model.fit(points, Y)

    print(model)
    model.save('{}_{}.mdl'.format(i, MODEL_TYPE))
